...
mat4 CreatePerspectiveMatrix(in float fov, in float aspect, in float near,
                             in float far) {
  mat4 m = mat4(0.0);
...
mat4 CamControl(vec3 eye, float pitch) {
  ...
  mat4 viewMatrix = mat4(
      vec4(xaxis.x, yaxis.x, zaxis.x, 0), vec4(xaxis.y, yaxis.y, zaxis.y, 0),
...
void main() {
  ...
  vec3 eye = vec3(0., 0.25 + 0.25 * cos(0.5 * time), -1.);
  mat4 projmat =
      CreatePerspectiveMatrix(50., dimensions.x / dimensions.y, 0.1, 10.);
  ...
  for (int i = 0; i < 24; ++i) {
    pos =
        vec4(p.x, 0.5 * fbm4(0.5 * vec2(eye.x + p.x, z + off)), eye.z + z, 1.);
    h = (vpmat * pos).y - p.y;
    if (h > lh) {
      d = abs(h);
