...
void main()
{
  ...
  vec3 f, c;
    ...
    sh = rotate2D(sun_pos, noise(uv+time*.25)*.3); //big noise on the sky
    ...
    if (uv.y > -.4) //drawing the sky
    {
        ...
        h = (hash12(floor(id.xx))*.5+.25)*(time+10.)*.25; //ring shift
        ...
        h = noise(t*vec2(.5,1)-vec2(time*.2,0)) //clouds
    ...
    if (uv.y < -.35) //drawing water
    {
        ...
        for (float i = 1.; i > -1.; i--) //drawing a wave and its neighbors from above and below
        {
            ...
            {
                lc = fract(u)-.5;
                lc.y = (lc.y+(sin(uv.x*12.-time*3.+id.y+i))*.25-i)*4.; //set the waveform and divide it into four strips
    ...
    f = mix(vec3(.7,.6,.2),vec3(0,1,0),sin(time*.2)*.5+.5); //color of the grass, changing from green to yellow and back again
        ...
        for (float y = 0.; y > -3.; y--)
          {
            for (float x = -2.; x <3.; x++)
            {
                ...
                lc-= vec2(.3,.5-h*.4);
                lc.x += sin(((time*1.7+h*2.-id.x*.05-id.y*.05)*1.1+id.y*.5)*2.)*(lc.y+.5)*.5;
    ...
    float T = sin(time*.5); //tree swing cycle
    ...
    if (abs(uv.x+tree_pos.x-.1-T*.1) < .6) // drawing the tree
    {
        ...
        for (float xs =0.;xs<4.;xs++) //drawing four layers of foliage
        {
            ...
            u += noise(u*vec2(2,1)+vec2(-time+xs*.05,0))*vec2(-.25,.1)*smoothstep (.5,-1.,u.y+.7)*.75; //leaves rippling in the wind
            ...
            f = hue((h+(sin(time*.2)*.5+.5))*.2).rgb-t.x; //color of the segment changes cyclically
