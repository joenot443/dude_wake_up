...
vec3 ramp(float t) {
  return t <= .5 ? vec3(1. - t * 1.4, .2, 1.05) / t
                 : vec3(.3 * (1. - t) * 2., .2, 1.05) / t;
}
vec2 polarMap(vec2 uv, float shift, float inner) {
  ...
  float py =
      (sqrt(uv.x * uv.x + uv.y * uv.y) * (1.0 + inner * 2.0) - inner) * 2.0;
...
void main() {
  ...
  float t = time;
  vec2 uv = coord / dimensions.yy;
  float ff = 1.0 - uv.y;
  uv.x -= (dimensions.x / dimensions.y - 1.0) / 2.0;
  vec2 uv2 = uv;
  uv2.y = 1.0 - uv2.y;
  ...
  vec3 c3 = c1 + c2 * vec3(1.0, 1.0, colorFactor);
  outputColor = vec4(c3, 1.0);
}