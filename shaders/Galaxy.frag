...
float noise(vec2 p) {
  ...
  float n = mix(mix(dot(shash2(i + vec2(0., 0.)), f - vec2(0., 0.)),
                ...
                mix(dot(shash2(i + vec2(0., 1.)), f - vec2(0., 1.)),
                    dot(shash2(i + vec2(1., 1.)), f - vec2(1., 1.)), u.x),
                u.y);
...
vec3 stars(vec3 ro, vec3 rd, vec2 sp, float hh) {
  ...
  for (float i = 0.0; i < m; ++i) {
    ...
    vec3 scol = mix(8.0 * h2, 0.25 * h2 * h2, s) *
                blackbody(mix(3000.0, 22000.0, h1 * h1));

    vec3 ccol = col + exp(-(mix(6000.0, 2000.0, hh) / mix(2.0, 0.25, s)) *
                          max(l - 0.001, 0.0)) *
                          scol;
    col = h3 < y ? ccol : col;
  }
