...
void zxspectrum_clash(  )
{
    vec2 pv = floor(coord.xy/LOWREZ);
    vec2 bv = floor(pv/8.0)*8.0;
    vec2 sv = floor(dimensions.xy/LOWREZ);
...
void zxspectrum_colors(  )
{
    ...
    if( mod(pv.x+pv.y,2.0)==1.0)
    {
		outputColor = vec4(fmap(vec4(floor(cs.rgb+vec3(0.5+(DITHER*0.3))),cs.a)),1.0);
    }
    else
    {  
		outputColor = vec4(fmap(vec4(floor(cs.rgb+vec3(0.5-(DITHER*0.3))),cs.a)),1.0);
