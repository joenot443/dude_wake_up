...
void main() {
    ...
    vec4 result = texture(tex, coord) * 0.227027;
    result += texture(tex, coord + tex_offset * blurRadius) * 0.1945946;
    result += texture(tex, coord - tex_offset * blurRadius) * 0.1945946;
    result += texture(tex, coord + tex_offset * 2.0 * blurRadius) * 0.1216216;
    result += texture(tex, coord - tex_offset * 2.0 * blurRadius) * 0.1216216;
    result += texture(tex, coord + tex_offset * 3.0 * blurRadius) * 0.054054;
    result += texture(tex, coord - tex_offset * 3.0 * blurRadius) * 0.054054;
