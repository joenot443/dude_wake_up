...
float fn(vec2 uv, vec2 suv) {
  float val = (texture(tex, uv).r + texture(tex, uv - vec2(BLUR_EPS, 0.0)).r +
               texture(tex, uv + vec2(BLUR_EPS, 0.0)).r) *
              0.333;
  val *= smoothstep(0.0, 1.0, clamp((3.0 - abs(suv.x - 0.5) * 8.0), 0.0, 1.0)) *
             0.8 +
         0.1;
  return val;
}
