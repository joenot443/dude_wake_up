...
void main()
{
  ...
  vec2 pos = (uv.xy - 0.5);
  vec2 cir = ((pos.xy * pos.xy + sin(uv.x * 18.0 * gamma + time) / 25.0 * sin(uv.y * 7.0 * delta + time * 1.5) / 1.0) + uv.x * sin(time) / 16.0 + uv.y * sin(time * 1.2) / 16.0);
