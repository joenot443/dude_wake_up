...
struct CommunityStrandTileView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            .onTapGesture {
                viewModel.loadSharedStrand(slug: strand.slug)
            }
            ...
            HStack(spacing: 8) {
                Button {
                    viewModel.voteOnStrand(slug: strand.slug, vote: "up")
                } label: {
                    HStack(spacing: 2) {
                ...
                Button {
                    viewModel.voteOnStrand(slug: strand.slug, vote: "down")
                } label: {
                    HStack(spacing: 2) {
        ...
        .cornerRadius(8)
        .onDrag {
            NSItemProvider(object: "shared_strand:\(strand.slug)" as NSString)
        }
        .contextMenu {
            Button("Copy Link") {
                let link = "https://nottawa.app/strands/\(strand.slug)"
