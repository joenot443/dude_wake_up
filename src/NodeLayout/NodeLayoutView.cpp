...
static const float NodeWidth = 20.0f;
...
void NodeLayoutView::draw() {
  ...
  ed::Begin("My Editor",
            ImVec2(ofGetScreenWidth() / 3.0f, ofGetScreenHeight() / 4.0f));
  ...
  for (std::shared_ptr<Shader> shader : shaders) {
    ...
    ed::BeginNode(nodeId);
    ImGui::Text(shader->idName().c_str());
  ...
  for (std::shared_ptr<ShaderChainer> shaderChainer : chainers) {
    ...
    ImGui::Text(shaderChainer->source->sourceName.c_str());
