...
void NodeLayoutView::debug()
{
  ...
  for (auto s : shaders)
  {
    auto id = idNodeMap[s->shaderId]->id;
...
void NodeLayoutView::draw()
{
  ...
  {
    auto shaderNode = nodeForShaderSourceId(shader->shaderId, NodeTypeShader, shader->name(), shader->supportsAux(), shader->supportsMask());
  ...
  for (std::shared_ptr<ShaderChainer> shaderChainer : chainers)
  {
    ...
    auto sourceNode = nodeForShaderSourceId(shaderChainer->source->id, NodeTypeSource, shaderChainer->name, false, false);
  ...
  for (auto const &x : nodeIdNodeMap)
  {
    ...
    if (shaderNode->shader->next != nullptr)
    {
      std::shared_ptr<Node> nextNode =
          idNodeMap[shaderNode->shader->next->shaderId];
    ...
    {
      std::shared_ptr<Node> auxNode = idNodeMap[shaderNode->shader->aux->shaderId];
      ed::LinkId nextShaderLinkId = uniqueId++;
    ...
    if (shaderNode->shader->supportsAux() &&
        shaderNode->shader->auxSource != nullptr)
    {
      std::shared_ptr<Node> auxNode = idNodeMap[shaderNode->shader->auxSource->id];
      ed::LinkId nextShaderLinkId = uniqueId++;
      shaderNode->auxNode = auxNode;
      auto shaderLink =
          std::make_shared<ShaderLink>(nextShaderLinkId, shaderNode, auxNode);
    ...
    // [Mask] Create a Link from Shader -> Shader
    if (shaderNode->shader->supportsMask() &&
        shaderNode->shader->mask != nullptr)
    {
      std::shared_ptr<Node> maskNode = idNodeMap[shaderNode->shader->mask->shaderId];
      ed::LinkId nextShaderLinkId = uniqueId++;
      shaderNode->maskNode = maskNode;
      auto shaderLink =
          std::make_shared<ShaderLink>(nextShaderLinkId, shaderNode, maskNode);
      shaderLink->type = LinkTypeMask;
      linksMap[nextShaderLinkId.Get()] = shaderLink;
      ed::Link(nextShaderLinkId, shaderLink->output->outputId,
               shaderLink->input->maskId);
    }

    // [Mask] Create a Link from Source -> Shader
    if (shaderNode->shader->supportsMask() &&
        shaderNode->shader->sourceMask != nullptr)
    {
      std::shared_ptr<Node> maskNode = idNodeMap[shaderNode->shader->sourceMask->id];
      ed::LinkId nextShaderLinkId = uniqueId++;
      shaderNode->maskNode = maskNode;
      auto shaderLink =
          std::make_shared<ShaderLink>(nextShaderLinkId, shaderNode, maskNode);
  ...
  for (auto selectedLinkId : selectedLinksIds)
  {
    ...
    if (shouldDelete && link != nullptr)
    {
      ...
      case LinkTypeAux:
      {
        ...
        {
          ShaderChainerService::getService()->breakSourceShaderAuxLink(link->output->source, link->input->shader);
        }
        else
        {
          // [Shader] -> [Aux]
      ...
      case LinkTypeMask:
      {
        ...
        if (link->output->type == NodeTypeSource)
        {
          ShaderChainerService::getService()->breakSourceShaderMaskLink(link->output->source, link->input->shader);
        }
        else
        {
          // [Shader] -> [Mask]
          ShaderChainerService::getService()->breakShaderMaskLink(
              link->output->shader, link->input->shader);
        }
        break;
      }
...
void NodeLayoutView::drawNode(std::shared_ptr<Node> node)
{
  ...
  else if (node->type == NodeTypeShader && ShaderChainerService::getService()->isTerminalShader(node->shader))
  {
    ...
    {
      VideoSourceService::getService()->addOutputWindowForChainer(ShaderChainerService::getService()->shaderChainerForShaderId(node->shader->shaderId));
    }
...
std::shared_ptr<Node> NodeLayoutView::nodeForShaderSourceId(std::string shaderSourceId, NodeType nodeType, std::string name, bool supportsAux, bool supportsMask)
{
...
void NodeLayoutView::handleDeleteNode(std::shared_ptr<Node> node)
{
  ...
  case NodeTypeShader:
    ...
    break;
  case NodeTypeSource:
    ShaderChainerService::getService()->removeShaderChainersForVideoSourceId(node->source->id);
      ...
      if (destNode->type == NodeTypeShader)
      {
        if (ed::AcceptNewItem())
        {
          ...
          if (sourceNode->type == NodeTypeSource)
          {
            ...
            {
              ShaderChainerService::getService()->linkVideoSourceToShaderAux(sourceNode->source, destNode->shader);
            }
            // If our Dest Pin is a Mask Pin
            else if (destPin->pinType == PinTypeMask)
            {
              ShaderChainerService::getService()->linkVideoSourceToShaderMask(sourceNode->source, destNode->shader);
            }
            // If our Dest Pin is a standard Input Pin
            else if (destPin->pinType == PinTypeInput)
            {
              ShaderChainerService::getService()->linkVideoSourceToShader(sourceNode->source, destNode->shader);
            }
          }
          ...
          if (sourceNode->type == NodeTypeShader)
          {
            ...
            if (destPin->pinType == PinTypeAux)
            {
              ShaderChainerService::getService()->linkShaderToShaderAux(
            ...
            {
              ShaderChainerService::getService()->linkShaderToShaderMask(
                  sourceNode->shader, destNode->shader);
...
void NodeLayoutView::drawPreviewWindow(std::shared_ptr<Node> node)
{
  auto pos = ed::GetNodePosition(node->id);
  pos = ed::CanvasToScreen(pos);
  pos.x = pos.x + (ed::GetNodeSize(node->id).x / ed::GetCurrentZoom()) / 2 - 80;
  pos.y = pos.y - 140;
  ImGui::SetNextWindowPos(pos);
  ImGui::SetNextWindowCollapsed(false);
...
void NodeLayoutView::drawZoomButtons()
{
  //  // Set the cursor to the bottom right of the window
  //  ImGui::SetCursorPos(ImVec2(ImGui::GetWindowSize().x - 50, ImGui::GetWindowSize().y - 50));
  //
  //  // Draw the zoom buttons
  //  if (CommonViews::IconButton(ICON_MD_ZOOM_IN, "zoom_in"))
  //  {
  //
  //  }
}
